...
cdef extern from "math.h":
    ...
    cdef extern float pow(float base, float exponent)
...
cdef class Tree:
    ...
    property threshold:
        def __get__(self):
            return floatp_to_ndarray(self.threshold, self.node_count)
    ...
    property value:
        def __get__(self):
            ...
            return np.PyArray_SimpleNewFromData(3, shape, np.NPY_FLOAT, self.value)

    property best_error:
        def __get__(self):
            return floatp_to_ndarray(self.best_error, self.node_count)

    property init_error:
        def __get__(self):
            return floatp_to_ndarray(self.init_error, self.node_count)
    ...
    def __cinit__(self, int n_features, object n_classes, int n_outputs,
                 Criterion criterion, float max_depth, int min_samples_split,
                 int min_samples_leaf, float min_density, int max_features,
    ...
    def __getstate__(self):
        ...
        d["threshold"] = floatp_to_ndarray(self.threshold, self.capacity)
        d["value"] = floatp_to_ndarray(self.value, self.capacity * self.value_stride)
        d["best_error"] = floatp_to_ndarray(self.best_error, self.capacity)
        d["init_error"] = floatp_to_ndarray(self.init_error, self.capacity)
    ...
    def __setstate__(self, d):
        ...
        cdef float* threshold = <float*> (<np.ndarray> d["threshold"]).data
        cdef float* value = <float*> (<np.ndarray> d["value"]).data
        cdef float* best_error = <float*> (<np.ndarray> d["best_error"]).data
        cdef float* init_error = <float*> (<np.ndarray> d["init_error"]).data
        ...
        memcpy(self.threshold, threshold, self.capacity * sizeof(float))
        memcpy(self.value, value, self.capacity * self.value_stride * sizeof(float))
        memcpy(self.best_error, best_error, self.capacity * sizeof(float))
        memcpy(self.init_error, init_error, self.capacity * sizeof(float))
        memcpy(self.n_samples, n_samples, self.capacity * sizeof(int))

    cdef void resize(self, int capacity=-1):
        """Resize all inner arrays to `capacity`, if < 0 float capacity."""
        ...
        cdef float* tmp_threshold = <float*> realloc(self.threshold, capacity * sizeof(float))
        if tmp_threshold != NULL: self.threshold = tmp_threshold

        cdef float* tmp_value = <float*> realloc(self.value, capacity * self.value_stride * sizeof(float))
        if tmp_value != NULL: self.value = tmp_value

        cdef float* tmp_best_error = <float*> realloc(self.best_error, capacity * sizeof(float))
        if tmp_best_error != NULL: self.best_error = tmp_best_error

        cdef float* tmp_init_error = <float*> realloc(self.init_error, capacity * sizeof(float))
    ...
    cpdef build(self, np.ndarray X, np.ndarray y,
        ...
        cdef float* buffer_value = <float*> malloc(self.value_stride * sizeof(float))
    ...
    cdef int add_split_node(self, int parent, int is_left_child, int feature,
        ...
        memcpy(self.value + offset_node, value, self.value_stride * sizeof(float))
    ...
    cdef void find_random_split(self, DTYPE_t* X_ptr, int X_stride,
        ...
        cdef float t, initial_error, error
        cdef float best_error = INFINITY, best_t = INFINITY
        ...
        for feature_idx from 0 <= feature_idx < n_features:
            ...
            n_left = criterion.update(0, c, y_ptr, y_stride, sample_mask_ptr)
...
cdef inline np.ndarray floatp_to_ndarray(float* data, int size):
    """Encapsulate data into a 1D numpy array of float's."""
    cdef np.npy_intp shape[1]
    shape[0] = <np.npy_intp> size
    return np.PyArray_SimpleNewFromData(1, shape, np.NPY_FLOAT, data)
